...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Maieuticoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Maieuticoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a Maieuticoin address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("maieuticoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("maieuticoin-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Maieuticoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <maieuticoinpubkey>\n"
            "Return information about <maieuticoinpubkey>.");
...
Value getstaketx(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->mapWallet.count(hash))
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
      			...
      			else if (!wtx.IsFromMe())
      			    entry.push_back(Pair("error", "Invalid transaction"));
      			else
      			{
                ...
                int64_t nTotal = wtx.GetValueOut();
        				int64_t nDebit = wtx.GetDebit();
        				int64_t nFee = nTotal - nDebit;
        				       				
        				entry.push_back(Pair("Original Amount", ValueFromAmount(nDebit)));
        				entry.push_back(Pair("PoS Reward", ValueFromAmount(nFee)));
        				entry.push_back(Pair("Total New Amount", ValueFromAmount(nTotal)));
      			}
    		}
    }
    else
    	entry.push_back(Pair("error", "Non-wallet transaction or invalid TX ID."));
    return entry;
}
