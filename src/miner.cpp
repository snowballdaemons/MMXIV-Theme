...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("maieuticoin-miner");
    ...
    while (!fShutdown)
    {
        ...
        if (pblock->SignBlock(*pwallet, nFees))
        {
            ...
            if (CheckStake(pblock.get(), *pwallet)) {
            	// We don't want to flood the network with a metric ton of PoS blocks and get a bunch of orphans, so we send them every 120-180 seconds maximum.
            	nextAllowedBlock = GetTime() + 120 + GetRandInt(60);
            }
            SetThreadPriority(THREAD_PRIORITY_LOWEST);
            MilliSleep(500);
        }
