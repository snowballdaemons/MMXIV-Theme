...
// MMXIV: optional setting to unlock wallet for block minting only;
...
// MMXIV: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // MMXIV: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // MMXIV: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // MMXIV: not avatar mode
                    {
...
// MMXIV: create coin stake transaction
...
// MMXIV: check 'spent' consistency between wallet and txindex
// MMXIV: fix wallet spent state according to txindex
...
// MMXIV: disable transaction (only for coinstake)
